...
GENRES_TOP10 = _get_top_genres(MOVIES_USED if MOVIES_USED else MOVIES, top_n=10)
...
def recommend():
    selected_ids = [int(x) for x in request.form.getlist('selected_movies') if str(x).isdigit()]
    if not selected_ids:
...
def api_search():
    ...
    global_min = min(m.get("year", 0) for m in pool if m.get("year") is not None) if pool else 1900
    global_max = max(m.get("year", 0) for m in pool if m.get("year") is not None) if pool else 2025
    if year_min is None:
        year_min = global_min
    if year_max is None:
        year_max = global_max
    ...
    no_filters = (q == "" and not selected_genres and year_min <= global_min and year_max >= global_max)
